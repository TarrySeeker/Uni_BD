...
/**
 * Тесты POST /api/storefront/v1/payments/paykeeper/init (docs/24 §2; зеркало
 * tbank/init). ANTI-ENUMERATION (единый 404), anti-tamper (сумма из тела
 * игнорируется — считает сервер), 409 при уже оплаченном/возвращённом заказе.
 *
...
const initPayment = vi.fn(async () => ({
  ...
  paymentUrl: 'http://x/mock/paykeeper/pay?orderId=ADMIK-2026-000042&invoiceId=mock-inv-1&amount=1500.00',
  status: 'sent',
  isMock: true,
}));
...
describe('storefront/payments/paykeeper/init', () => {
  beforeEach(() => setEnv());
  afterEach(() => {
  ...
  it('верный токен → 200, invoice + isMock (anti-tamper: сумма из тела игнор)', async () => {
    const token = orderAccessToken(EXISTING_ID, { APP_PASSWORD: 'token-secret-for-test' });
    const r = await init({ orderNumber: EXISTING_NUMBER, accessToken: token, sum: '1' });
    expect(r.status).toBe(200);
    expect(initPayment).toHaveBeenCalledTimes(1);
    expect((r.data as { invoiceId: string }).invoiceId).toBe('mock-inv-1');
...
describe('storefront/payments/paykeeper/init — 409 уже оплачен', () => {
  beforeEach(() => setEnv());
  afterEach(() => {
    process.env = { ...ORIGINAL };
    vi.resetModules();
