...
export interface ProductListRow {
  ...
  /**
   * Презентационный кеш атрибутов товара (цвет/пол/…) — источник фасетов сетки
   * каталога. Читается из products.attributes_cache (ADR-007), а не джойном по
   * EAV: фасеты нужны на КАЖДОЙ строке списка, и джойн на страницу товаров
   * превратился бы в N запросов к product_attributes.
   */
  attributesCache: Record<string, unknown>;
  /**
   * УНИКАЛЬНЫЕ метки размеров из имён активных вариантов — для фильтра по
   * размеру в сетке каталога. При матрице «цвет × размер» имена вариантов
   * повторяются (2 цвета × 4 размера = 8 вариантов, 4 метки), поэтому дубли
   * схлопнуты (uniqueSizes): фасет сравнивает метки точной строкой.
   */
  sizes: string[];
  createdAt: Date;
}
