...
export function LanguagesForm({ config }: { config: LocaleConfig }) {
  ...
  return (
    <div>
      ...
      {/*
        🔴 АУДИТ №31. Прежде здесь стояло ЛОЖНОЕ утверждение: «витрина этот набор
        ещё не читает, выключенный язык не исчезнет с витрины». Витрина набор
        читает давно (settings.i18n.locales → enabledLocalesFrom), и выключенный
        язык она как раз УБИРАЕТ: переключатель его не показывает, а запрос его
        адреса уводится 307-редиректом на язык по умолчанию. Плашка утверждала
        обратное — владелец получал ровно противоположный ожидаемому результат.

        Второе (правдивое) ограничение — асимметрия: убрать язык с витрины набор
        может, а ДОБАВИТЬ язык, для которого у витрины нет словаря, — не может
        (whitelist LOCALES витрины отфильтрует его молча). Набор при этом
        намеренно НЕ ограничен справочником одной витрины: админка и БД — общая
        платформа, обслуживающая магазины с разными витринами и разными наборами
        языков (мультитенантность). Поэтому свобода набора сохранена, а про
        асимметрию честно предупреждает storefrontSupportHint.
      */}
      <div role="note" className="mb-4 rounded border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
        ...
        <p className="mt-2">{t('settings.languagesForm.storefrontSupportHint')}</p>
      </div>
