  ...
  return runStorefront(
    ...
    async ({ cors }) => {
      ...
      if (!found || !verifyOrderAccess(found.order, { token: accessToken, email })) {
        // Транспорт остаётся 404 (единый ответ «не найдено» для несуществующего и
        // неавторизованного), доменная причина едет рядом — витрина покажет свой
        // перевод, а не серверную строку (аудит №3/№6).
