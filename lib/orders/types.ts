...
export interface Order {
  ...
  grandTotal: string;
  /**
   * Снимок валюты на момент заказа. Источник — ЕДИНЫЙ резолвер
   * `resolveOrderCurrency` (эффективные настройки магазина, env лишь фолбэк), то
   * есть ровно то же значение, что показано покупателю на витрине (аудит №32).
   * Значение НЕ пересчитывается при смене базовой валюты магазина: у уже
   * созданных заказов остаётся валюта, в которой они были оформлены.
   */
  currency: string;

  // ---- Снимок валюты ОТОБРАЖЕНИЯ (мультивалюта, миграция 0059) ----
  /**
   * Валюта, в которой покупатель СМОТРЕЛ цены на витрине (переключатель ₽/€), —
   * ISO-4217. null = заказ оформлен в базовой валюте (переключатель не трогали)
   * либо заказ старше миграции 0059.
   *
   * 🔴 СПРАВОЧНОЕ ПОЛЕ. Деньги списываются в `currency` по `grandTotal`; эти три
   * поля нужны, чтобы через неделю разобрать претензию «мне показывали 540 €»:
   * курс ЦБ меняется ежедневно и нигде больше не сохраняется. Ни платёжный путь,
   * ни сверка с эквайером, ни возвраты от них не зависят.
   */
  displayCurrency: string | null;
  /**
   * Курс на момент заказа: единиц БАЗОВОЙ валюты за 1 единицу валюты отображения
   * (EUR 88.76020000 → 1 € = 88,7602 ₽, значит цена_€ = цена_₽ / rate). Строка
   * NUMERIC(18,8). null — снимка нет.
   */
  displayRate: string | null;
  /**
   * Итог заказа в валюте ОТОБРАЖЕНИЯ (справочно). Строка NUMERIC(14,2) — ДЕНЬГИ
   * (евро), НЕ копейки. Считается СЕРВЕРОМ как grandTotal / displayRate — именно
   * от итога, а не суммированием округлённых позиций (иначе разойдётся на
   * единицу валюты). null — снимка нет.
   */
  ...
  /**
   * Время ПОСЛЕДНЕЙ инициации платежа — когда по заказу выставили счёт (0058).
   * Ставится вместе с payment_ref любым провайдером. null — не инициировался
   * (или заказ старше миграции). Нужен витрине, чтобы отличить «оплатил и
   * вернулся раньше вебхука» от «не платил вовсе»: в первом случае предлагать
   * оплату нельзя — это второе списание.
   */
