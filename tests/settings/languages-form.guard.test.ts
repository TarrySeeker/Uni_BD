...
describe('LanguagesForm — честное предупреждение про витрину', () => {
  /**
   * 🔴 АУДИТ №31 переписал это ожидание. Прежний текст утверждал, что «витрина
   * этот набор ещё не читает и выключенный язык с неё не исчезнет» — ЛОЖЬ:
   * витрина набор читает (settings.i18n.locales → enabledLocalesFrom) и
   * выключенный язык как раз убирает 307-редиректом на дефолт. Слово «витрина»
   * из текста ушло намеренно (владельцу понятнее «сайт»), поэтому сторожим не
   * подстроку-жаргон, а СУТЬ: сказано про эффект на сайте и про язык по
   * умолчанию. Отдельный тест про правдивость — languages-storefront-honesty.
   */
  it('форма объясняет эффект набора на сайте и роль языка по умолчанию', () => {
    const labels = ruUnder('settings.languagesForm');
    expect(labels).toMatch(/сайт|витрин/i);
    expect(labels).toMatch(/язык по умолчанию/i);
  });
});
