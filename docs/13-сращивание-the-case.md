...
> **Регистронезависимость — не только для атрибутов товара, но и для enum-значений
> настроек.** Тот же принцип нормализации (нижний регистр, обрезка пробелов, при
> необходимости латиница/кириллица и `-`↔`_`) адаптер/витрина применяют при сопоставлении
> значений-перечислений, которые владелец задаёт в настройках или которые приходят в
> полях заказа/доставки/оплаты: тип соцсети, способ доставки/оплаты, коды статусов и т.п.
> Значение может прийти в другом регистре (`Telegram` vs `telegram`) — сопоставление не
> должно ломаться из-за этого; неизвестное после нормализации значение → безопасный дефолт,
> а не «пропажа» блока и не исключение.

> **`[object Object]` в интерфейсе = объект отрендерен как строка.** Значит в JSX/шаблон
> попал объект/массив вместо примитива (частая причина — поле API вложеннее, чем ждёт
> витрина). Мок-данные в тестах обязаны повторять **точную форму** реального ответа API
> (вложенность, типы полей по `docs/21`), иначе тест зелёный, а на витрине — `[object Object]`.
