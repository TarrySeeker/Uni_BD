...
Но фильтр секретов НЕ должен вырезать шаблон окружения `.env.example` — он обязан
оставаться на стенде как эталон переменных. Паттерн исключения — только точный
`--exclude '.env'` (совпадает по имени и НЕ трогает `.env.example`); никогда не
расширяй его до `--exclude '.env*'`, иначе шаблон пропадёт со стенда.

## Анти-дрейф стенд↔репозиторий (каталог витрины вне git)
Каталог витрины физически лежит на диске (`Dockerfile`, статика/`src/`), но в git этого
репозитория он НЕ отслеживается (`git ls-files storefront/` пуст). Отсюда два риска:
- **Дрейф.** Ручная правка витрины прямо на стенде НЕ фиксируется в репозитории и теряется
  при следующем `rsync`/деплое. Любое изменение витрины делай в исходниках; чинить «по
  месту» на стенде запрещено.
- **Артефакты инстанса — под контроль версий.** Конфиги, специфичные для инстанса
  (эталонный `serve.json` раздачи статики, `docker-compose.override`, seed-профиль, README
  инстанса), держи в git репозитория ИНСТАНСА и восстанавливай из него, а не из копии на
  стенде. Секреты в них не хранить — только ссылки на env.

После деплоя сверь, что выгруженное на стенд соответствует источнику: непустой diff по
отслеживаемым файлам = незакоммиченный дрейф, устрани его в исходниках до продолжения.
