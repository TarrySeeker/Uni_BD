...
for f in "${FILES[@]}"; do
  ...
  # ЯВНО ОБЪЯВЛЕННОЕ ОТСТУПЛЕНИЕ ОТ ПРАВИЛА.
  #
  # Часть правил нельзя соблюсти в принципе. Канонический случай: CHECK-ограничение
  # невозможно РАСШИРИТЬ, не пересоздав (два CHECK на колонке действуют по «И», и
  # старый всё равно отвергнет новое значение). Так расширяется перечень
  # допустимых платёжных провайдеров при подключении каждого нового эквайера.
  #
  # Чтобы такие миграции не приходилось «пропускать» правкой самого гейта, файл
  # объявляет отступление САМ — строкой-маркером в комментарии:
  #
  #   -- check-migrations: allow drop-constraint (причина отступления)
  #
  # Маркер именно в файле, а не в списке исключений внутри скрипта: он виден на
  # ревью рядом с кодом и не переживает файл. Разрешается ТОЧЕЧНО одно правило,
  # остальные продолжают действовать.
  allow_drop_constraint=0
  if grep -qiE '^[[:space:]]*--[[:space:]]*check-migrations:[[:space:]]*allow[[:space:]]+drop-constraint' "${f}"; then
    allow_drop_constraint=1
    warn "$(basename "${f}"): объявлено отступление «drop-constraint» (проверьте обоснование в шапке файла)"
  fi
  ...
  # 3. DROP CONSTRAINT (можно объявить отступление маркером — см. выше)
  if [ "${allow_drop_constraint}" -eq 0 ]; then
    check_rule "${content}" "${f}" \
