/**
 * Cron-сверка платежей PayKeeper (порт tbank/cron.ts на инвойсную модель).
 *
 * Проблема: PayKeeper не шлёт гранулярных статусов, а потерянный колбэк оставляет
 * реально оплаченный заказ в payment_status='pending'. Этот воркер по «зависшим»
 * paykeeper-заказам дёргает getInvoiceStatus (PaymentService.reconcilePayment) и
 * ИДЕМПОТЕНТНО доводит статус.
 *
 * Чистая, тестируемая логика с инъекцией deps. По умолчанию deps — реальные (sql +
 * PaymentService), в тестах подменяются моками.
 *
 * Гарантии: идемпотентность (recordWebhookEvent, UNIQUE (invoice_id, status)),
...
const RECONCILE_LOCK_KEY = 'paykeeper:reconcile-pending';

/** Кандидат на сверку: «зависший» оплачиваемый paykeeper-заказ. */
export interface PendingPaymentCandidate {
  id: string;
  number: string;
  /** invoice_id PayKeeper (orders.payment_ref) — обязателен (фильтр IS NOT NULL). */
  invoiceId: string;
...
/**
 * «Зависшие» оплачиваемые paykeeper-заказы для сверки:
 *   payment_provider='paykeeper', payment_status='pending', есть payment_ref,
 *   заказ не отменён/возвращён, создан за последние 3 дня.
 * Сумма из grand_total → копейки воркером. ORDER BY created_at LIMIT.
 */
export async function findPendingPaykeeperPayments(
  ...
  >`
    SELECT o.id, o.number, o.payment_ref, o.grand_total
      FROM orders o
     WHERE o.payment_provider = 'paykeeper'
  ...
  return rows.map((r) => ({
    ...
    invoiceId: String(r.payment_ref),
...
function defaultReconcileDeps(): ReconcileDeps {
  return {
    ...
    findCandidates: findPendingPaykeeperPayments,
    reconcile: (c) =>
      new PaymentService()
        .reconcilePayment({
          orderId: c.id,
...
/**
 * reconcile-pending-paykeeper (каждые 15 мин): по «зависшим» paykeeper-заказам
 * дёргает getInvoiceStatus и доводит статус. Идемпотентно, устойчиво, сериализовано
