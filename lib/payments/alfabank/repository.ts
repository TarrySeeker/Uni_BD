/**
 * Репозиторий модуля payments/alfabank (порт paykeeper/repository.ts на таблицу
 * alfabank_payment_log с ключом идемпотентности UNIQUE (order_ref, status)).
 * БД-зависимый слой:
 ...
 *   • setPaymentRefAndProvider — сохранить orderId/провайдера на заказе после register;
 *   • findOrderIdByRef / getOrderGrandTotalById — поиск заказа/суммы для колбэка/сверки.
 *
...
/** Поля для записи события в alfabank_payment_log (идемпотентная вставка). */
...
/**
 * Идемпотентно пишет событие колбэка. UNIQUE (order_ref, status); дубликат
 * (повторная доставка колбэка Альфа-Банка) → inserted=false.
 */
export async function insertPaymentLog(input: PaymentLogInput): Promise<PaymentLogResult> {
  const rows = await sql<{ id: string }[]>`
    INSERT INTO alfabank_payment_log (
      order_id, order_ref, status, amount_kop, is_mock, raw_payload, ip
    ) VALUES (
      ${input.orderId}, ${input.orderRef}, ${input.status},
      ${input.amountKop ?? null}, ${input.isMock ?? false},
      ${input.rawPayload ? sql.json(input.rawPayload as Record<string, never>) : null},
      ${input.ip ?? null}
    )
    ON CONFLICT (order_ref, status) DO NOTHING
    RETURNING id
  `;
...
export async function markPaymentLogProcessed(id: string): Promise<void> {
  await sql`UPDATE alfabank_payment_log SET processed = true WHERE id = ${id}`;
}

// -----------------------------------------------------------------------------
// Смена payment_status (без Server Actions) — порт paykeeper applyPaymentStatusTx.
// -----------------------------------------------------------------------------
...
async function applyPaymentStatusTx(
  ...
  if (
    ...
    logger.warn(
      'alfabank: пропущен переход payment_status на отменённом/возвращённом заказе — требуется ручная сверка/возврат',
      { module: 'payments/alfabank', orderId, orderStatus, from, to },
    );
    return false;
  }
...
/**
 * АТОМАРНО обрабатывает событие колбэка Альфа-Банка В ОДНОЙ транзакции (sql.begin):
 ...
 *
 * Ключ идемпотентности — UNIQUE (order_ref, status) → ON CONFLICT DO NOTHING.
 */
  ...
  return await sql.begin<RecordWebhookResult>(async (tx: TransactionSql) => {
    const rows = await tx<{ id: string }[]>`
      INSERT INTO alfabank_payment_log (
        ...
        ${input.log.orderId}, ${input.log.orderRef}, ${input.log.status},
    ...
    await tx`UPDATE alfabank_payment_log SET processed = true WHERE id = ${id}`;
    return {
      inserted: true,
      processed,
      applied: processed,
...
/**
 * Ищет id заказа по orderId Альфа-Банка (orders.payment_ref). Приоритетный способ
 * поиска заказа на колбэке: payment_ref пишется при регистрации. Возвращает null,
 * если заказ с таким payment_ref не найден (вызывающий пробует фолбэк по номеру).
 */
export async function findOrderIdByRef(orderRef: string): Promise<string | null> {
  const rows = await sql<{ id: string }[]>`
    SELECT id FROM orders WHERE payment_ref = ${orderRef} LIMIT 1
  `;
  return rows[0]?.id ?? null;
}

/**
 * Возвращает СЕРВЕРНЫЙ grand_total заказа (рублёвая строка) для сверки/аудита суммы.
 * Источник истины — БД. null → заказ не найден. Вынесено отдельно (лёгкий SELECT).
 */
