...
const envSchema = z.object({
  ...
  CDEK_CREATE_ENABLED: z
    ...
    .default('true')
    .transform((v) => v === 'true' || v === '1'),
  // Регистрировать накладную СРАЗУ при оформлении, БЕЗ ожидания оплаты (магазины
  // без онлайн-кассы). Дефолт false: штатно накладная создаётся только после
  // оплаты, и магазины, которым режим не нужен, не должны молча получить другое
  // поведение при обновлении кита.
  ...
  // Разрешить mock-оплату В PRODUCTION (демо-стенд). По умолчанию ЗАПРЕЩЕНО.
  //
  // Зачем флаг вообще нужен: в mock оплата не ходит в банк, а фоновая доводка
  // платежей помечает заказ «оплачен». Потерянный TBANK_PASSWORD на боевом
  // сервере превращал магазин в раздачу товара — молча, без единого признака в
  // интерфейсе. Поэтому прод без ключей теперь ПАДАЕТ, а не деградирует, и
  // демо-стенд обязан заявить о себе явно.
  TBANK_ALLOW_MOCK: z
    .enum(['true', 'false', '1', '0'])
    .default('false')
    .transform((v) => v === 'true' || v === '1'),
