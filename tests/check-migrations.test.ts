...
describe('check-migrations.sh — реальные миграции 0001..0024', () => {
  ...
  // ⚠️ Таймаут задан явно и с запасом. Дефолтные 5 с молча привязывали тест к
  // ЧИСЛУ миграций: линтер порождает пачку подпроцессов на каждый файл, и на 35
  // миграциях полный обход занимает ~7 с — тест начинал падать по таймауту, хотя
  // сам линтер отрабатывал успешно. У развёрнутых магазинов миграций уже за
  // шестьдесят, так что порог, не зависящий от их количества, обязателен.
  it('все db/migrations/*.sql аддитивны (exit 0)', { retry: 2, timeout: 120_000 }, () => {
