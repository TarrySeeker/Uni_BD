...
function integrationItems(input: ReadinessInput): ReadinessItem[] {
  ...
  // --- Отправка почты --------------------------------------------------------
  //
  // Предупреждение, а не блокер: магазин без писем работает, просто уведомления
  // о заказах приходится рассылать вручную. (В сборке с личным кабинетом этот
  // пункт становится блокером — там без почты нельзя ни подтвердить адрес, ни
  // восстановить пароль.)
