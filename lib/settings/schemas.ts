...
/**
 * ВНУТРЕННИЙ путь — то есть адрес, который гарантированно остаётся на нашем домене.
 *
 * 🔴 ПОЧЕМУ НЕДОСТАТОЧНО `startsWith('/')`, КАК БЫЛО РАНЬШЕ.
 * Проверка ставилась против ОПЕЧАТОК («catolog» без слэша), и про внешние
 * адреса речи не было. Но `//evil.com` тоже начинается со слэша — а браузер
 * трактует `//host` как PROTOCOL-RELATIVE URL, то есть как чужой домен с
 * текущей схемой. В результате ссылка баннера главной или пункт меню шапки
 * становились ОТКРЫТЫМ РЕДИРЕКТОМ: покупатель жмёт «Каталог» и уезжает на
 * сторонний сайт, а строка в настройках выглядит внутренним путём.
 *
 * Сюда же:
 *  • обратный слэш — `/\evil.com` браузеры нормализуют к `//evil.com`;
 *  • управляющие символы — `/\t/evil.com` после нормализации даёт то же самое.
 *
 * Настройки правит администратор, поэтому это не дыра «на любой запрос», но
 * администратор не обязан знать про protocol-relative URL, а значение может
 * приехать и из чужого бэкапа настроек. Отсекаем на валидаторе.
 */
function isInternalPath(v: string): boolean {
  if (!v.startsWith('/')) return false;
  // Второй символ «/» или «\» → адрес ведёт на другой хост, а не внутрь сайта.
  if (v.length > 1 && (v[1] === '/' || v[1] === '\\')) return false;
  // Обратный слэш и управляющие символы выкидываются/нормализуются браузером,
  // поэтому строка с ними означает не то, что видит администратор.
...
const imageRefSchema = z
  ...
  .min(1)
  .refine(
    // isInternalPath, а не startsWith('/'): иначе '//evil.com' сошло бы за
    // внутренний путь и картинка грузилась бы с чужого домена (тот же изъян,
    // что был в hrefSchema — см. комментарий к isInternalPath).
    (v) => isInternalPath(v) || isHttpUrl(v),
 ...
 * (без протокола), пустую строку, опасный «javascript:», а также внешние
 * адреса, замаскированные под внутренний путь (см. isInternalPath).
 */
const hrefSchema = z
  .string()
  .trim()
  .min(1)
  .refine(
    (v) => isInternalPath(v) || isHttpUrl(v) || /^(?:mailto:|tel:)\S+$/i.test(v),
