...
/**
 * Разделы ВЫЕЗЖАЮЩЕГО МЕНЮ — то же разворачивание, что у browseGroups, и по той же
 * причине: дерево API приходит с техническим корнем `catalog`, который САМ разделом
 * не является. Показать его пунктом — значит спрятать весь каталог за одним
 * «Каталог +» вместо списка разделов, как на эталоне (docs/41 §2: Твилли, Платки
 * и шарфы, Одежда…, Декор… — каждый со своим «+»).
 *
 * Отдельное имя (а не прямой вызов browseGroups из шапки) — чтобы правило меню
 * можно было менять независимо от карточек главной: у них разные заказчики
 * (эталон меню vs. эталон главной), хоть сегодня разворачивание и совпадает.
 */
export function menuSections(tree: CategoryDto[]): CategoryDto[] {
  return browseGroups(tree);
}
